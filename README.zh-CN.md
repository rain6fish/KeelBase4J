...
KeelBase 协议才是唯一真源，而不是恰好最先实现它的那份 TypeScript 代码。兼容性由「复现主仓里那批语言无关的向量」来证明——那是任何别的实现都该用的同一批文件。
...
**138 条测试**全绿（协议 58 · 运行时 54 · 生成器 16 · springai 10）。下面每个主张都能真跑出来——四个脚本，前三个不需要模型：
